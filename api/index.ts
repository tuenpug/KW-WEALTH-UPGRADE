...
// Configure yahoo-finance2 to use a common User-Agent
if (yf && yf.setOptions) {
  yf.setOptions({
    queue: {
      concurrency: 1
...
// API routes FIRST - Register synchronously so they are available immediately on export
app.get("/api/stock/:symbol", async (req, res) => {
  const { symbol } = req.params;
  const { period1, period2 } = req.query;

  console.log(`API Request: /api/stock/${symbol}?period1=${period1}&period2=${period2}`);
  ...
  try {
    ...
    console.log(`Fetching from Yahoo: ${symbol} from ${p1.toISOString()} to ${p2.toISOString()}`);
    ...
    console.log(`Yahoo result received for ${symbol}`);
    ...
    if (!result || !result.quotes || result.quotes.length === 0) {
      console.warn(`No quotes found for ${symbol}`);
...
if (import.meta.url === `file://${process.argv[1]}` || process.env.NODE_ENV !== "production") {
  ...
  // In production (Vercel), we still need to serve static files if it's not handled by Vercel rewrites
  const distPath = path.join(process.cwd(), 'dist');
  if (fs.existsSync(distPath)) {
    app.use(express.static(distPath));
  }
